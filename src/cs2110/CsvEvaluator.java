...
public class CsvEvaluator {
    ...
    public static void evaluateCsv(CSVParser parser, CSVPrinter printer) throws IOException {
        ...
        // A mapping of the coordinates of cells we have seen so far to their numerical values.
        VarTable vars = new MapVarTable();
        int rowNum = 1; // Start row numbering at 1

        for (CSVRecord csvRecord : parser) {
            for (int colNum = 0; colNum < csvRecord.size(); colNum++) {
                String cell = csvRecord.get(colNum);
                String cellCoord = colToLetters(colNum + 1) + rowNum; // Column letters start at 1

                if (cell.startsWith("=")) { // Checking if it needs to be evaluated
                    try {
                        double val = RpnParser.parse(cell.substring(1), defs).eval(vars);
                        printer.print(String.valueOf(val)); // Otherwise, its contents are replaced
                        // by Java's String representation of the formula's value.
                        vars.set(cellCoord, val);
                    } catch (Exception e) {
                        printer.print("#N/A"); // If the formula cannot be evaluated for any reason,
                        // its contents are replaced by "#N/A" in the output.
                    }
                } else {
                    printer.print(cell);
                    try {
                        double value = Double.parseDouble(cell);
                        vars.set(cellCoord, value);
                    } catch (NumberFormatException e) {
                        // If it's not a number, we don't store it in vars
                        // It stays as a formula itself
                    }
                }
            }
            printer.println(); // Move to the next line after printing all cells in the record
    ...
    public static String colToLetters(int n) {
        ...
        // 0 is represented by an empty String.
        if (n == 0){ // Base Case
            return "";
        }
        else {
